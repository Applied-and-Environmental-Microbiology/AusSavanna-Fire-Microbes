...
# Required files
otus <- fread("data/prepared_reads/OTUs.txt")
taxonomy <- fread("data/taxonomy/taxa_OTUs.txt")
sequences <- readDNAStringSet("data/prepared_reads/OTUs.fasta")
...
# 4,900 appears to be a reasonable lowest read depth
...
# The relationship is moderate (r2 = 0.3), suggesting that the sequencing depth
# does strongly influences the OTU richness
...
otus_srs <- SRS(
  ...
  data = otus_2 %>% column_to_rownames(var = 'OTU_ID'),
...
alpha_diversity_fungi = otus_srs %>%
  # Convert to long format
  pivot_longer(
    -otu_id,
    names_to = "sample_id",
  ...
  group_by(sample_id) %>%
  summarise(
    richness_fungi = n_distinct(otu_id),
    shannon_fungi = shannon_div(count)
  ) %>%
  mutate(
...
fwrite(
  otus %>%
    ...
    select(otu_id = OTU_ID, all_of(leaf_samples)) %>%
    # Remove the low abundance sample_ids
    select(-all_of(low_abundance_sample_ids)) %>%
...
fwrite(
  otus %>%
    # Select only leaf or soil sample_ids
    select(otu_id = OTU_ID, all_of(soil_samples)) %>%
    filter(rowSums(select(., -otu_id) > 0) >= 2),  # <-- at least 2 sample_ids
...
ordination_results_leaf <- site_coords_leaf %>%
  rename(
    !!paste0("pco1_", prop1_leaf, "_leaf_fungi") := PC1,
    !!paste0("pco2_", prop2_leaf, "_leaf_fungi") := PC2
  ) %>%
  select(sample_id, everything())
ordination_results_soil <- site_coords_soil %>%
  rename(
    !!paste0("pco1_", prop1_soil, "_soil_fungi") := PC1,
    !!paste0("pco2_", prop2_soil, "_soil_fungi") := PC2
  ) %>%
  select(sample_id, everything())
...
fwrite(
  metadata_beta_div,
  "../../data/sample_metadata_fungi.txt", 
  sep = "\t"
  )

# Move the distance matrices to the main data directory
system("mv data/distances_leaf/distance-matrix.tsv ../../data/distance_fungi_leaf.txt")
system("mv data/distances_soil/distance-matrix.tsv ../../data/distance_fungi_soil.txt")
